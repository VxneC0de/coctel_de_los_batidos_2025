...
<?php
  ...
  if (isset($_SESSION['who'])) {
    $user_id = $_SESSION['who'];
    $sql = "SELECT COUNT(DISTINCT id_product_cart) as unique_products FROM cart WHERE id_user_cart = '$user_id' AND status = 1";
    $consult = mysqli_query($connection, $sql);
    $row = mysqli_fetch_assoc($consult);
    $unique_products = isset($row['unique_products']) ? (int)$row['unique_products'] : 0;
    echo "<script>console.log('Unique products: " . $unique_products . "');</script>"; // Añadir esto para depuración
  } else {
    $unique_products = 0;
  }
?>
<script>
  document.addEventListener('DOMContentLoaded', function() {
    console.log('Unique products from PHP: <?php echo $unique_products; ?>'); // Añadir esto para depuración
    const uniqueProductsCount = <?php echo json_encode($unique_products); ?>;
    document.querySelector('.cart_icon span').textContent = !isNaN(uniqueProductsCount) ? uniqueProductsCount : 0;
  });
</script>
...
function updateCartQuantity() {
    const cartItems = document.querySelectorAll('.cart_items .cart_item');
    console.log('Number of unique cart items: ' + cartItems.length);
    
    const cartIcon = document.querySelector('.cart_icon span');
    cartIcon.textContent = cartItems.length;

    console.log('Updated cart icon textContent to: ' + cartIcon.textContent);
}
...
document.querySelectorAll('.add_to_cart').forEach(button => {
    button.addEventListener('click', function() {
        ...
        if (existingCartItem) {
            ...
            const newQuantity = parseInt(existingQuantity.textContent) + parseInt(productData.quantity);
            existingQuantity.textContent = newQuantity;

            const existingPriceSpan = existingCartItem.querySelector('.item_details_price span');
            const newPrice = (parseFloat(existingPriceSpan.textContent) + (parseFloat(productData.price) * parseInt(productData.quantity))).toFixed(2);
            ...
            console.log(`Updated existing product: ${productData.name}, new quantity: ${newQuantity}`);
        } else {
            ...
            cartItem.innerHTML = `
                ...
                <div class="item_details">
                    ...
                    <div class="item_details_price">
                        <strong>Bs. <span>${(parseFloat(productData.price) * parseInt(productData.quantity)).toFixed(2)}</span></strong>
                    </div>
                </div>
                <div class="qty">
                    <span>-</span>
            ...
            const hiddenInputsHTML = `
                ...
                <input type="hidden" name="price_cart[]" value="${(parseFloat(productData.price) * parseInt(productData.quantity)).toFixed(2)}">
...
document.addEventListener('DOMContentLoaded', function() {
    const uniqueProductsCount = <?php echo json_encode($unique_products); ?>;
    console.log('Unique products from PHP: ' + uniqueProductsCount);

    const cartIcon = document.querySelector('.cart_icon span');
    cartIcon.textContent = !isNaN(uniqueProductsCount) ? uniqueProductsCount : 0;

    console.log('Updated cart icon textContent on load to: ' + cartIcon.textContent);
});
